...
//			if (mgr != null) mgr.LuaGC();
		...
		{
			String uuid = GetTime ().ToString() + UUIDNum;
			return uuid;
		}
		...
		/// unity坐标转换为fgui的屏幕坐标
		/// </summary>
		/// <returns>The world to user interface screen.</returns>
		/// <param name="scvec">Scvec.</param>
		public static Vector2 VecWorldToUIScreen(Vector3 scvec)
		{
			Vector3 result = Camera.main.WorldToScreenPoint(scvec);
			result.y = Screen.height - result.y;
			Vector2 pt=GRoot.inst.GlobalToLocal (result);
			return pt;
		}
